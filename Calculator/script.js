let screenOutput = document.getElementById('display-output');
    ...
    input.addEventListener('click', (e) => {
        ...
        if (currInput == 'AC') {
            ...
            if (currLength > 1) {
                ...
                if (prevInput == '+' || prevInput == '-' || prevInput == '*' || prevInput == '/' || prevInput == '%' || prevInput == '.') {
                    if (currInput == '+' || currInput == '-' || currInput == '*' || currInput == '/' || currInput == '%' || currInput == '.') {
                        ans = ans.substring(0, ans.length - 1);
                        ans = ans + currInput;
                        screenOutput.value = ans;
                        return;
